/***
   ...
   In this specific code deals with the case in which temperature and light only affect prey dynamics and the survival and duration of the egg and larval period. 
***/
...

{
	...
	for (int j = 0; j < 4; j++) {
		for (i = 0; i < cohort_no[j]; i++) {
			...
			//ingestion
			Az = phiarr(MEANTEMP, EAM, TR) * pow(mass, EI / kB * (1 / (MEANTEMP + kel) - 1 / (TR + kel))) * AMAX * pow((mass / WOPT * exp(1 - mass / WOPT)), ALPHA);
			Ab = phiarr(MEANTEMP, EAM, TR) * B1 * pow(mass, B2 + EI / kB * (1 / (MEANTEMP + kel) - 1 / (TR + kel)));


			etazL = max(0, Az * zoopL); // netagive value protection)
			etab = max(0, Ab * bent);

			hand = D1 * pow(mass, D2) / phiSS(MEANTEMP, EAD, EDD, TR, THD);
			...
			Em = phiarr(MEANTEMP, EAM, TR) * pow(mass, EI / kB * ((double)1 / (MEANTEMP + kel) - (double)1 / (TR + kel))) * RHO1 * pow(pop[j][i][indx] + pop[j][i][indy], RHO2);
			Eg = Ea - Em;
	...
	if (fmod(time, 365) > 364 - TINY && fmod(time, 365) < 364 + TINY) { // early mutant
		...
		if (fmod(time+1, BIFPERIOD * 365) > MUTYEAR * 365 - TINY && fmod(time+1, BIFPERIOD * 365) < (MUTYEAR + MUTPERIOD - 1) * 365 + TINY) {
			...
			for (i = 0; i < SD + 1; i++) {
				...
				if (i == 0)			pop[2][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * 1.0 / (1 + exp(-(0 - SM))) * MUTINIT;// first reproduction day to make distribution add to 1
				else if (i == SD) 		pop[2][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * (1.0 - 1.0 / (1 + exp(-(SD - 1 - SM)))) * MUTINIT; // last reproduction day to make distribution add to 1
				else 				pop[2][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * (1.0 / (1 + exp(-(i - SM))) - 1.0 / (1 + exp(-(i - 1 - SM)))) * MUTINIT;

				popIDcard[2][NewCohort + i][IDbtime] = time + i; // egg release time
				popIDcard[2][NewCohort + i][IDbage] = (AF + EPSMIN + EPSP * phiarr(temp, EAE, TR)); // egg and larvea duration
	...
	if (fmod(time, 365) > 365 - TINY || fmod(time, 365) < TINY) {
		...
		if (fmod(time, BIFPERIOD * 365) > MUTYEAR * 365 - TINY && fmod(time, BIFPERIOD * 365) < (MUTYEAR + MUTPERIOD - 1) * 365 + TINY) {
			...
			for (i = 0; i < SD + 1; i++) {
				...
				if (i == 0)			pop[1][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * 1.0 / (1 + exp(-(0 - SM))) * MUTINIT;// first reproduction day to make distribution add to 1
				else if (i == SD) 		pop[1][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * (1.0 - 1.0 / (1 + exp(-(SD - 1 - SM)))) * MUTINIT; // last reproduction day to make distribution add to 1
				else 				pop[1][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * (1.0 / (1 + exp(-(i - SM))) - 1.0 / (1 + exp(-(i - 1 - SM)))) * MUTINIT;

				popIDcard[1][NewCohort + i][IDbtime] = time + i; // egg release time
				popIDcard[1][NewCohort + i][IDbage] = (AF + EPSMIN + EPSP * phiarr(temp, EAE, TR)); // egg and larvea duration
	...
	if (fmod(time, 365) > 1 - TINY && fmod(time, 365) < 1 + TINY) { // late mutant
		...
		if (fmod(time -1, BIFPERIOD * 365) > MUTYEAR * 365 - TINY && fmod(time - 1, BIFPERIOD * 365) < (MUTYEAR + MUTPERIOD - 1) * 365 + TINY) {
			...
			for (i = 0; i < SD + 1; i++) {
				...
				if (i == 0)			pop[3][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * 1.0 / (1 + exp(-(0 - SM))) * MUTINIT;// first reproduction day to make distribution add to 1
				else if (i == SD) 		pop[3][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * (1.0 - 1.0 / (1 + exp(-(SD - 1 - SM)))) * MUTINIT; // last reproduction day to make distribution add to 1
				else 				pop[3][NewCohort + i][number] = phiSS(temp, EAS, EDS, TR, THS) * (1.0 / (1 + exp(-(i - SM))) - 1.0 / (1 + exp(-(i - 1 - SM)))) * MUTINIT;

				popIDcard[3][NewCohort + i][IDbtime] = time + i; // egg release time
				popIDcard[3][NewCohort + i][IDbage] = (AF + EPSMIN + EPSP * phiarr(temp, EAE, TR)); // egg and larvea duration
