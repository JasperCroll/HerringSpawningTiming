/***
   ...
   In this specific code deals with the case in which temperature and light only affect herring dynamics and the survival and duration of the egg and larval period.
***/
...

{
	...
	envgrad[0] = 1;//time
	envgrad[1] = max( 0, MEANLIGHTPROD * phiSS(MEANTEMP, EAP, EDP, TR, THP) * PZ) -  phiarr(MEANTEMP, EAR, TR) * RZ * zoopL - consZL/SCALE; // zooplankton
	envgrad[2] = max(0, phiSS(MEANTEMP, EAP, EDP, TR, THP) * PB ) - phiarr(MEANTEMP, EAR, TR) * RB * bent - consB * VB/SCALE; // benthos
